...
export class SidebarComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this.profileImageurl = this.authService.currentUserValue.profileImageUrl;
	}
		...
		if (this.authService.currentUserValue) {
			this.sidebarItems = ROUTES.filter((sidebarItem) => {
				if (sidebarItem.title === 'MENUITEMS.ADMIN.TEXT') {
					sidebarItem.isVisible = this.isUserRoleExists('Admin') || this.isUserRoleExists('Student');

					for (const item of sidebarItem.submenu) {
						item.isVisible = this.isUserRoleExists('Admin');
					}
				}
				if (sidebarItem.title === 'MENUITEMS.CALENDAR.TEXT' || sidebarItem.title === 'MENUITEMS.NOTISES.TEXT') {
					sidebarItem.isVisible = true;
				}
				if (sidebarItem.title === 'MENUITEMS.TEACHER.TEXT') {
					sidebarItem.isVisible = this.isUserRoleExists('Teacher');
				}
